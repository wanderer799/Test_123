    ...
    if form.validate_on_submit():
        ...
        for file in form.choose_file.data:
            ...
            if "signal" in data.keys():
                ...
                sig = list(
                    map(int, data["ECGDataMessageClass"]["CDataChannelA"].split(" "))
                )
                sig = standardise(
                    sig,
                    int(data["ECGDataMessageClass"]["SamplingFrequency"]),
                    int(
                        re.findall("\d+", data["ECGDataMessageClass"]["ADCResolution"])[
                            0
                        ]
                    ),
                )
            ...
            plot_ecg(
                ts=outs["ts"],
                raw=file1,
                filtered=outs["filtered"],
                rpeaks=outs["rpeaks"],
                templates_ts=outs["templates_ts"],
                templates=outs["templates"],
                heart_rate_ts=outs["heart_rate_ts"],
                heart_rate=outs["heart_rate"],
                path="static/images/file{}".format(abc) + ".png",
                show=False,
            )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    marker=dict(color="#fc0505"),
                    textfont=dict(color="#fc0505"),
                )
            )
            fig.add_trace(
                go.Scatter(
                    ...
                    marker=dict(color="#aaad0a"),
                    textfont=dict(color="#aaad0a"),
                )
            )
            fig.add_trace(
                go.Scatter(
                    x=np.arange(0, len(file1)),
                    y=file1,
                    name="Signal",
                    textposition="top center",
                    marker=dict(color="#00cf7c"),
                )
            )
            fig.update_layout(
                paper_bgcolor="#173048",
                plot_bgcolor="#173048",
                font_color="#aaad0a",
                title=title,
            )
            ...
            if (
                "Sinus Tachychardia" in events
                or "Sinus bradycardia" in events
                or "Sinus Arrhythmia" in events
            ):
            ...
            plotly.offline.plot(
                fig,
                filename="static/file{}".format(abc) + ".html",
                auto_open=False,
...
def con():
    ...
    fig = px.line(
        ...
        y=concat,
        title="ECG Strips Combined",
        color_discrete_map={"Average": "#fc0d0d", "Raw": "#fc0d0d"},
    )
    fig.add_trace(
        go.Scatter(
            x=x,
    ...
    fig.update_layout(
        paper_bgcolor="#173048", plot_bgcolor="#173048", font_color="#aaad0a"
    )
    ...
    print("fig type is:", type(fig))
    plotly.offline.plot(
        fig, filename="static/concat.html", auto_open=False, include_plotlyjs="cdn"
    )
    ...
    plotly.offline.plot(
        fig, filename="static/piechart.html", auto_open=False, include_plotlyjs="cdn"
    )
    ...
    return render_template(
        "concat2.html", beats=beats, hrv=np.mean(hrv).round(), eventss=eventss
    )
